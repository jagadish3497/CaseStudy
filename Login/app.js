...
app.get('/borrowerlogin', function(req, res) {
  res.render('borrowerlogin', {
    user: req.user
  });
});
...
app.post('/ownerlogin', function(req, res, next) {
 ...
 app.post('/borrowerlogin', function(req, res, next) {
  passport.authenticate('local', function(err, user, info) {
    if (err) return next(err)
    if (!user) {
      return res.redirect('/borrowerlogin')
    }
    req.logIn(user, function(err) {
      if (err) return next(err);
      return res.redirect('/');
  ...
  async.waterfall([
    ...
    function(token, done) {
      User.findOne({ email: req.body.email }, function(err, user) {
        ...
        user.resetPasswordExpires = Date.now() + 30000; // 1 hour
    ...
    function(token, user, done) {
      ...
      var mailOptions = {
        to: user.email,
        from: 'jagadeesh.krishna123@gmail.com',
        subject: 'Puppi On Demand Password Reset',
